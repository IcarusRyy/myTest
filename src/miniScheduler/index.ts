// 引入小顶堆封装代码
import { push, pop, peek } from "./miniHeap"

// 浏览器提供的API，获取从 time origin （当前文档生命周期的开始节点时间）之后到当前调用经过的时间，它以一个恒定的速率慢慢增加，不会收到系统时间的影响
let getCurrentTime = () => performance.now()

// Scheduler优先级划分，数字越小，优先级越高，0表示没有优先级
const NoPriority = 0
...
// Scheduler 根据优先级设置对应的timeout时间， 越小代表越紧急
...
// 是否有正在执行的 requestHostTimeout，它会在 requestHostTimeout 执行前设为 true，cancelHostTimeout 和 handleTimeout 中设为 false
...
function requestHostCallback(callback) {
  // 将callback函数存为全局变量，传入的是flushWork
...
// 借助MessageChannel 让出线程 告诉浏览器 等空闲了再执行任务队列
...
function performWorkUntilDeadline() {
  // scheduledHostCallback 就是 flushWork这个函数
  if (scheduledHostCallback !== null) {
    ...
    try {
      hasMoreTask = scheduledHostCallback(hasTimeRemaining, currentTime)
    } finally {
      if (hasMoreTask) {
        // 如果在一个时间切片里没有完成所有的任务，那就执行schedulePerformWorkUntilDeadline， 让出线程，等待浏览器空闲了再继续
        schedulePerformWorkUntilDeadline()
      } else {
...
function workLoop(hasTimeRemaining, initialTime) {
  ...
  // 检查timerQueue中的任务，将到期的任务转移到taskQueue中
  ...
  while (currentTask !== null) {
    ...
    if (typeof callback === "function") {
      ...
      // React中单个任务执行的时候，也可以被打断，如果单个任务执行的时候被打断了，会返回一个函数
      // 这个任务被打断了
  ...
  if (currentTask === null) {
    ...
    // 如果taskQueue空了，timerQueue中的最先执行的任务还没有到时间，那么就执行一个requestHostTimeout定时器，保证准时执行
...
// 检查timerQueue中的任务，将到期的任务转到taskQueue中
